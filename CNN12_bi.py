...
Arquitectura CNN (12 capas) - Clasificación Binaria 
=============================================================================
"""
...
def sleepiness_cnn():
    ...
    # Capa de salida: 2 neuronas para clasificación binaria
...
model.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=0.001, clipnorm=1.0), loss=tf.keras.losses.CategoricalCrossentropy(), metrics=['accuracy'])
model.summary()
    ...
    def __getitem__(self, idx):
        ...
        for path, es_dup in zip(batch_paths, batch_dup):
            try:
                ...
                senal_procesada = data['senal_final'].flatten().astype(np.float32)

...
# =============================================================================
def sleepiness_cnn(input_shape=(4320000, 1), fs=100):
...
class SignalGeneratorMAT(Sequence):
    ...
    def __getitem__(self, idx):
        ...
        if len(batch_x) == 0:
            if self.weights_dict is not None:
                return (np.empty((0, self.target_length, 1), dtype=np.float32),
                        np.empty((0,), dtype=np.float32),
                        np.empty((0,), dtype=np.float32))
            return np.empty((0, self.target_length, 1), dtype=np.float32), np.empty((0,), dtype=np.float32)
...
checkpoint = ModelCheckpoint(filepath='mejor_modelo_mwt.keras', monitor='val_loss', save_best_only=True, mode='min', verbose=1)
...
model.save(f'modelos_entrenados/modelo_cnn4_{NOMBRE_ETIQUETA}.keras')

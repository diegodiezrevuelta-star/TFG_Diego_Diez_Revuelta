...
# =============================================================================
#  ARQUITECTURA COMPLETA (CNN + GRU BINARIA)
# =============================================================================
def build_complete_sleep_model_binario(n_sequences, insize_per_ep, nunit=64, dropout_rnn=0.3, num_clases=NUM_CLASES):
    ...
    # 4. Salida de clasificación binaria 
    model.add(layers.Dense(num_clases, activation='softmax', kernel_initializer='he_normal'))
...
class SignalGeneratorMAT(Sequence):
    ...
    def __getitem__(self, idx):
        ...
        # Retorno de seguridad por si falla el batch completo
        if len(batch_x) == 0:
            return (np.empty((0, self.n_sequences, self.insize_per_ep, 1), dtype=np.float32), 
                    np.empty((0, NUM_CLASES), dtype=np.float32))

        X = np.array(batch_x)
        # --- CORRECCIÓN CRÍTICA AQUÍ: Reestructurar de señal plana a épocas ---
